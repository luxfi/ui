/**
 * @luxfi/ui Tailwind Configuration
 *
 * Extends @hanzo/ui's Tailwind preset with Lux-specific tokens.
 * Uses unified --ui-* CSS variables (same across all brands).
 *
 * Usage in your app's tailwind.config.js:
 *
 *   import luxPreset from '@luxfi/ui/tailwind.config.lux'
 *
 *   export default {
 *     presets: [luxPreset],
...
import { luxTailwindTheme } from './system'

const luxPreset: Partial<Config> = {
  darkMode: 'class',
  theme: {
    extend: {
      // Use Lux design tokens (spacing, typography, etc.)
      ...luxTailwindTheme,
