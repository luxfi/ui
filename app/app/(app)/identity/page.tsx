...
import { HANZO_REGISTRY_ABI, AI_TOKEN_ABI, CONTRACT_ADDRESSES, NETWORK_NAMES } from '@/lib/contracts'
...
export default function IdentityPage() {
  ...
  const hasEnoughBalance = aiBalance && stakeRequirement ? aiBalance >= stakeRequirement : false
  const needsApproval = allowance && stakeRequirement ? allowance < stakeRequirement : true
  const canProceed = isConnected && identityName.length > 0 && validateName(identityName) && isAvailable && hasEnoughBalance
            ...
            {isConnected && aiBalance !== undefined && (
              <div className="mt-4">
                <p className="text-sm text-muted-foreground">
                  AI Balance: <span className="font-mono font-semibold">{formatEther(aiBalance)} AI</span>
                </p>
              </div>
            )}
          </CardContent>
            ...
            <CardContent className="space-y-6">
              <div className="space-y-2">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  <Input
                    ...
                    value={identityName}
                    onChange={(e) => setIdentityName(e.target.value.toLowerCase())}
                    className="font-mono"
                  />
                ...
                {identityName.length > 0 && (
                  <div className="flex gap-2 mt-2">
                    ...
                    {isAvailable !== undefined && validateName(identityName) && (
                      <Badge variant="outline" className={isAvailable ? "bg-green-50 text-green-700 border-green-200" : "bg-red-50 text-red-700 border-red-200"}>
              ...
              <div className="bg-muted/50 rounded-lg p-4 space-y-2">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-semibold">{pricingInfo.tier}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-muted-foreground">Base Price:</span>
                  <span className="text-sm font-semibold">{pricingInfo.price}</span>
                </div>
                {referrer && (
                ...
                {stakeRequirement && (
                  <>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-mono font-bold text-lg">{formatEther(stakeRequirement)} AI</span>
                    </div>
                  </>
                )}
              </div>
              ...
              {!hasEnoughBalance && stakeRequirement && (
                <Alert>
                  <AlertDescription>
                    Insufficient AI balance. You need {formatEther(stakeRequirement)} AI but only have {formatEther(aiBalance || 0n)} AI.
                    <br />
                    <a href="/faucet" className="underline font-semibold mt-1 inline-block">
              ...
              {isClaimSuccess && (
                <Alert className="bg-green-50 border-green-200">
                  <AlertDescription className="text-green-700">
                    Identity @{identityName}.{networkName} successfully registered!
                  </AlertDescription>
                </Alert>
              )}
            </CardContent>
            ...
            <CardFooter className="flex flex-col gap-2">
              {needsApproval && canProceed && (
                <Button
                  ...
                  disabled={!canProceed || isApprovePending || isApproveConfirming}
                  className="w-full"
                >
                  {isApprovePending || isApproveConfirming ? 'Approving...' : 'Approve AI Tokens'}
                </Button>
              )}
              ...
              {!needsApproval && canProceed && (
                <Button
                  ...
                  className="w-full"
                >
                  {isClaimPending || isClaimConfirming ? 'Claiming...' : `Claim Identity (${formatEther(stakeRequirement || 0n)} AI)`}
                </Button>
              )}
            </CardFooter>
          </Card>
          ...
          <CardContent>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between py-2 border-b">
                <span className="text-muted-foreground">Single character (1)</span>
                <span className="font-mono font-semibold">100,000 AI</span>
              </div>
              <div className="flex justify-between py-2 border-b">
                <span className="text-muted-foreground">Two characters (2)</span>
                <span className="font-mono font-semibold">10,000 AI</span>
              </div>
              <div className="flex justify-between py-2 border-b">
                <span className="text-muted-foreground">Three characters (3)</span>
                <span className="font-mono font-semibold">1,000 AI</span>
              </div>
              <div className="flex justify-between py-2 border-b">
                <span className="text-muted-foreground">Four characters (4)</span>
            ...
            <div className="mt-4 p-3 bg-muted/50 rounded-lg">
              <p className="text-xs text-muted-foreground">
                💡 <strong>Referrer Bonus:</strong> Use a valid referrer to get 50% off the registration price!
              </p>
            </div>
          </CardContent>
        </Card>
